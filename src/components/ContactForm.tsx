...
// Read EmailJS keys from Vite env
...
export function ContactForm() {
  ...
  const onSubmit = async (data: ContactFormData) => {
    ...
    try {
      // Sanity-check env keys
      if (!SERVICE_ID || !TEMPLATE_ID || !PUBLIC_KEY) {
        throw new Error('EmailJS env keys are missing. Check your .env and vite-env.d.ts');
      }
      ...
      const templateParams = {
        ...
        // Optional if you added these in your template:
        // reply_to: data.email,
      ...
      if (res.status !== 200) {
        throw new Error(`EmailJS returned non-200: ${res.status} ${res.text}`);
      }

      setFormState('success');
      setStatusMessage('Thanks! Your message has been sent.');

      // Reset after a moment and refocus first field
      setTimeout(() => {
        reset();
        setFocus('fullName');
        setFormState('idle');
    ...
    <div className="max-w-2xl mx-auto">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
        ...
        <div>
          ...
          />
          {errors.fullName && (
            <p id="fullName-error" className="mt-2 text-destructive flex items-center gap-2" role="alert">
        ...
        <div>
          ...
          />
          {errors.email && (
            <p id="email-error" className="mt-2 text-destructive flex items-center gap-2" role="alert">
              <AlertCircle className="w-4 h-4 flex-shrink-0" />
        ...
        <div>
          ...
          />
          {errors.subject && (
            <p id="subject-error" className="mt-2 text-destructive flex items-center gap-2" role="alert">
              <AlertCircle className="w-4 h-4 flex-shrink-0" />
        ...
        <div>
          ...
          />
          {errors.message && (
            <p id="message-error" className="mt-2 text-destructive flex items-center gap-2" role="alert">
              <AlertCircle className="w-4 h-4 flex-shrink-0" />
        ...
        {/* Privacy Note */}
        <p className="text-muted-foreground">I'll only use your info to reply.</p>

        {/* Status */}
        <div className="min-h-[1.5rem]" aria-live="polite" aria-atomic="true">
          {statusMessage && (
            <div className={`flex items-center gap-2 ${formState === 'success' ? 'text-success' : 'text-destructive'}`}>
              {formState === 'success' ? (
